...
async def main():
    ...
    im2 = InfoMarket2(per_run=2, total=100, quote="EUR", verbose=True, only_positions=False)
    # 1) Warm-up della giornata (tutte le finestre) e salvataggio:
    # path = await im2.build_currency_store(
    #     store_dir="currencyStorePerDay",
    #     file_prefix="currencyOfDay",                 # default
    #     ranges=("NOW","5M","30M","1H","4H","24H","48H","7D", "30D","90D")
    # )
    # print("snapshot salvato:", path)


        # 2) poi parti col watcher veloce (short ranges), che aggiorna
    #    e ti invia batch come stream_async:
    async for batch in im2.watch_currency_store(
        store_dir="currencyStorePerDay",
        file_prefix="currencyOfDay",
        short_ranges=("NOW","15M"),    # default
        full_ranges=("NOW","5M","15M","30M","1H","4H","24H","48H","7D","30D"),
        full_every_hours=4,
        poll_seconds=2,
        batch_size=10,          # <<— produce 2 item per yield
        stop_after=2,       # o un numero per fermarti dopo N giri
        full_at_start=False,   # parte immediatamente col ciclo veloce
    ):
    #     # Qui ricevi SUBITO 2 currency aggiornate.
    #     pass
    # Puoi passarle alla tua AI o salvarle altrove.
        print("Batch ricevuto:", len(batch))
        pass
